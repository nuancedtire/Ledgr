...
function parseCSV(text: string): Transaction[] {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const isValidDate = !isNaN(startedDate.getTime());
    const iso = isValidDate ? startedDate.toISOString() : '';
    const type = fields[0];
...
export function getSummaryStats(): SummaryStats {
  ...
  for (const t of txns) {
    ...
    if (t.product === 'Current') {
      ...
      totalFees += t.fee;
      if (t.balance !== null) currentBalance = t.balance;
    } else if (t.product === 'Savings') {
      if (t.balance !== null) lastSavings = t.balance;
    }
  }
  ...
  _statsCache = {
    ...
    dateRange: {
      from: isFinite(minDate) ? new Date(minDate).toISOString().slice(0, 10) : '',
      to: isFinite(maxDate) ? new Date(maxDate).toISOString().slice(0, 10) : '',
    },
