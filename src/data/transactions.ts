 ...
 * Memoized to avoid repeated filtering of the full dataset.
 */
 ...
 * Optimized with module-level memoization and single-pass traversal.
 */
export function getSummaryStats(): SummaryStats {
  ...
  _statsCache = {
    ...
    dateRange: {
      from: new Date(minDate).toISOString().slice(0, 10),
      to: new Date(maxDate).toISOString().slice(0, 10),
    },
...
export function getClientData(): ClientData {
  ...
  const activeTxns = [...getActiveTransactions()].sort((a, b) => b.startedDate.getTime() - a.startedDate.getTime());
