...
async function startUpload(file: File): Promise<void> {
  ...
  try {
    ...
    // Same-origin API call — Worker handles /upload and /status/*
    ...
    renderSteps(1, statuses);
    await pollWorkflow(instanceId, statuses);
  } catch (err) {
    setStatus(
...
async function pollWorkflow(
  ...
  for (let i = 0; i < 120; i++) {
    ...
    try {
      const resp = await fetch(`/status/${instanceId}`);
      ...
      if (activeIdx === -1)
        activeIdx = STEPS.findIndex((s) => !statuses[s.id]);
      renderSteps(
      ...
      if (data.status === 'complete') {
        ...
        setStatus(
          '\u2705 Done! New data committed. Site will rebuild in ~1 minute.',
        );
        return;
      }
